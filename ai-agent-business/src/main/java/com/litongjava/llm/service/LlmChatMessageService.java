  ...
  public void parse(JSONArray reqMessages) {
    ...
    for (int i = 0; i < reqMessages.size(); i++) {
      ...
      if ("system".equals(role)) {
        ...
        messages.add(new UniChatMessage().role(role).content(content.toString()));
        OpenAiChatMessage openAiUniChatMessage = new OpenAiChatMessage(role, content.toString());
        openAiChatmessages.add(openAiUniChatMessage);
          ...
          for (int j = 0; j < contentsArray.size(); j++) {
            ...
            if ("image_url".equals(type)) {
              ...
              if (url.startsWith("data:image/")) {
                ...
                ChatMessageContent multiContent = new ChatMessageContent();
              ...
              messages.add(new UniChatMessage().role(role).content(contentObj.getString("text")));
            }
          }
        }
      }
