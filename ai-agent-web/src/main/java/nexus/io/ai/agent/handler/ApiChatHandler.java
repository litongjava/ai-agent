...
public class ApiChatHandler {
  ...
  public HttpResponse like(HttpRequest request) {
    ...
    boolean exists = Db.exists(AiAgentBaseTableNames.llm_chat_history, "id", questionId);
    if (!exists) {
      return response.fail(RespBodyVo.fail("invalid quesion id"));
    }
    exists = Db.exists(AiAgentBaseTableNames.llm_chat_history, "id", answerId);
    if (!exists) {
    ...
    sql = String.format(sql, AiAgentBaseTableNames.llm_chat_history, AiAgentBaseTableNames.llm_chat_history);
    ...
    messageText.append("app.env:").append(EnvUtils.appEnv()).append("\r\n");
